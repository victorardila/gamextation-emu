...
from src.ui.components.controls.keyboard_interactive import (
    KeyboardInteractive,
    OctagonButton,
    RectangularButton,
)
...
class Consoles(QWidget):
    ...
    def style_labels(self, custom_font):
        ...
        for label, text in labels.items():
            ...
            label.setText(text)
            label.setStyleSheet(
                "background-color: transparent; color: white; font-size: 26px;"
            )
        self.label_tittle_options.setStyleSheet(
            "background-color: rgba(0, 0, 0, 0.5); border-radius: 10px; color: white; font-size: 26px;"
        )
        self.label_functios_avaliable.setStyleSheet(
            "background-color: rgba(0, 0, 0, 0.5); border-radius: 10px; color: white; font-size: 26px;"
        )
    ...
    def style_options(self, custom_font):
        ...
        self.option_controller.style(
            self.SVGS_PATH[0], QSize(40, 40), "Control", QColor("white")
        )
        self.option_keyboard.style(
            self.SVGS_PATH[1], QSize(40, 40), "Teclado", QColor("white")
        )
    ...
    def update_controller_selection(self):
        ...
        if self.option_keyboard.isChecked():
            ...
            self.interactive_preview_content.layout().addWidget(
                self.keyboard_interactive
            )
        elif self.option_controller.isChecked():
            # Instancia un nuevo GamepadInteractive
            self.joystick_interactive = GamepadInteractive()
            self.interactive_preview_content.layout().addWidget(
                self.joystick_interactive
            )
