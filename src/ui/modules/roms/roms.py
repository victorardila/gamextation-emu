...
class Roms(QWidget):
    ...
    game_data_selected = pyqtSignal(dict)  # Señal para emitir el juego seleccionado

    def __init__(self, parent=None):
        super().__init__(parent)
        self.loader = RenderLoader(self)
        self.loader.show_centered(
            self
        )  # Mostrar el loader centrado sobre el widget Roms
        self.setupUi()
    ...
    def load_games(self):
        ...
        image_urls = [
            game["cover_image"] for category in self.games.values() for game in category
        ]
    ...
    def on_image_loaded(self, image_url, image):
        ...
        if all(
            isinstance(game["cover_image"], BytesIO)
            for category in self.games.values()
            for game in category
        ):
    ...
    def apply_styles(self):
        ...
        self.label_module.setStyleSheet(
            "background-color: transparent; color: white; font-size: 26px; font-weight: bold;"
        )
        self.label_module.setFont(custom_font)
        self.label_name_game.setStyleSheet(
    ...
    def load_cover_games(self):
        ...
        for i in range(total_slots_default):
            ...
            cover_game.game_selected.connect(
                self.handle_game_selected
            )  # Conectar la señal
            if i % 2 == 0 and index_desktop < len(games_desktop):
                index_desktop = self.find_next_available_index(
                    games_desktop, used_desktop_games, index_desktop
                )
                ...
                index_browser = self.find_next_available_index(
                    games_browser, used_browser_games, index_browser
                )
    ...
    def handle_optimizer_hidden(self):
        ...
        self.label_optimize.setStyleSheet(
            "background-color: transparent; color: #00ff00; font-size: 20px; font-weight: bold;"
        )
        self.label_optimize.setVisible(True)
        self.label_optimize.setText("Recursos optimizados")
        self.label_optimize.setFont(
    ...
    def handle_game_selected(self, game_data):
        """Maneja la selección de un juego."""
        # Creo una llave para el juego seleccionado y a esa llave le asigno el valor de game_data
        game_selected = {"Game": game_data}
        self.game_data_selected.emit(game_selected)
