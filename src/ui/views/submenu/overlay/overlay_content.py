...
class OverlayContent(QWidget):
    ...
    def init_submenu(self):
        ...
        self.button_menu_bar.clicked.connect(
            self.toggle_sidebar
        )  # Conectar el botón al método
        self.button_optimize.clicked.connect(
            self.show_graphics_optimizer
        )  # Conectar el botón al método
    ...
    def load_module(self, message=None):
        ...
        # Conectar la señal optimizer_hidden del módulo Roms a un método en OverlayContent
        if key == "Juegos":
            self.module.game_data_selected.connect(self.game_data_received.emit)
    ...
    def apply_sidebar_styles(self):
        ...
        self.label_logo_menu.setStyleSheet(
    ...
    def apply_button_styles(self):
        ...
        self.button_menu_bar.style(
            "fa.angle-right", QSize(32, 32), "Desplegar", "white"
        )
    ...
    def toggle_sidebar(self):
        ...
        if self.sidebar_open:
            ...
            self.button_menu_bar.style(
                "fa.angle-left", QSize(32, 32), "Cerrar", "white"
            )
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.button_menu_bar:
            if event.type() == QEnterEvent.Enter:
                # Handle hover event
                self.toggle_sidebar()
                self.set_button_menu_bar_opacity(
                    1.0
                )  # Set opacity to 100% when hovered
    ...
    def show_graphics_optimizer(self):
        ...
        self.graphics_optimizer.setGeometry(
            self.content_menu.width()
            - widget_rect.width()
            - 0,  # 20 píxeles desde el borde derecho
